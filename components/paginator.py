...
class Paginator:
    ...
    def add_line(self, line: typing.Optional[str]):
        ...
        if len(self.pages[-1]) + ll > self.page_size:
            self.pages.append("")
        self.pages[-1] += self.prefix + line + self.suffix + self.sep  # type: ignore


class PaginatorView(discord.ui.View):
    def __init__(
        self, paginator, author: typing.Union[discord.User, discord.Member], **kwargs
    ):
    ...
    async def update_message(self, interaction: discord.Interaction):
        self.page_number.label = self.page + 1  # type: ignore
    ...
    @discord.ui.button(
        label=1, style=discord.ButtonStyle.primary, disabled=True  # type: ignore
    )
